defmodule Mix.Tasks.Phx.Gen.Mdlive do
  ...
  Overall, this generator will add the following files to `lib/`:
    ...
    * a LiveComponent in `lib/app_web/live/components/modal_component/modal_component.ex`
  ...
  Alternatively, the `--context-app` option may be supplied to the generator:
      ...
      mix phx.gen.mdlive Sales User users --context-app warehouse
  ...
  module name, for example:
      ...
      mix phx.gen.mdlive Sales User users --web Sales
  ...
  def run(args) do
    if Mix.Project.umbrella?() do
      Mix.raise("mix phx.gen.mdlive can only be run inside an application directory")
    end
  ...
  defp files_to_be_generated(%Context{schema: schema, context_app: context_app}) do
    ...
    [
      ...
      {:new_eex, "modal_component.ex",
       Path.join([web_prefix, "live", "components", "modal_component", "modal_component.ex"])},
  ...
  defp copy_new_files(%Context{} = context, binding, paths) do
    ...
    Mix.Phoenix.copy_from(paths, "priv/templates/phx.gen.mdlive", binding, files)
  ...
  def print_shell_instructions(%Context{schema: schema, context_app: ctx_app} = context) do
    ...
    Make the following helper functions available in your Templates:
  ...
  def inputs(%Mix.Phoenix.Schema{} = schema) do
    Enum.map(schema.attrs, fn
      ...
      {key, :integer} ->
        ~s(<%= MDF.number_input f, #{inspect(key)}, label: [text: gettext\("#{
          Phoenix.Naming.humanize(Atom.to_string(key))
        }"\)] %>)

      {key, :float} ->
        ~s(<%= MDF.number_input f, #{inspect(key)}, input: [step: "any"], label: [text: gettext\("#{
          Phoenix.Naming.humanize(Atom.to_string(key))
        }"\)] %>)
      ...
      {key, :boolean} ->
        ~s(<%= MDF.checkbox f, #{inspect(key)}, label: [text: gettext\("#{
          Phoenix.Naming.humanize(Atom.to_string(key))
        }"\)] %>)

      {key, :text} ->
        ~s(<%= MDF.textarea f, #{inspect(key)}, label: [text: gettext\("#{
          Phoenix.Naming.humanize(Atom.to_string(key))
        }"\)] %>)

      {key, :date} ->
        ~s(<%= MDF.date_select f, #{inspect(key)}, label: [text: gettext\("#{
          Phoenix.Naming.humanize(Atom.to_string(key))
        }"\)] %>)

      {key, :time} ->
        ~s(<%= MDF.time_select f, #{inspect(key)}, label: [text: gettext\("#{
          Phoenix.Naming.humanize(Atom.to_string(key))
        }"\)] %>)

      {key, :utc_datetime} ->
        ~s(<%= MDF.datetime_select f, #{inspect(key)}, label: [text: gettext\("#{
          Phoenix.Naming.humanize(Atom.to_string(key))
        }"\)] %>)
      ...
      {key, {:array, :integer}} ->
        ~s(<%= MDF.multiple_select f, #{inspect(key)}, ["1": 1, "2": 2], label: [text: gettext\("#{
          Phoenix.Naming.humanize(Atom.to_string(key))
        }"\)] %>)

      {key, {:array, _}} ->
        ~s(<%= MDF.multiple_select f, #{inspect(key)}, ["Option 1": "option1", "Option 2": "option2"], label: [text: gettext\("#{
          Phoenix.Naming.humanize(Atom.to_string(key))
        }"\)] %>)

      {key, _} ->
        ~s(<%= MDF.text_input f, #{inspect(key)}, label: [text: gettext\("#{
          Phoenix.Naming.humanize(Atom.to_string(key))
        }"\)] %>)
    end)
  end

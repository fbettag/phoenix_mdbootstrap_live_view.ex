  ...
  def inputs(%Mix.Phoenix.Schema{} = schema) do
    Enum.map(schema.attrs, fn
      {_, {:references, _}} ->
        nil
      {key, :integer} ->
        ~s(<%= MDF.number_input f, #{inspect(key)}, label: [text: gettext\("#{key}"\)] %>)
      {key, :float} ->
        ~s(<%= MDF.number_input f, #{inspect(key)}, input: [step: "any"], label: [text: gettext\("#{key}"\)] %>)
      {key, :decimal} ->
        ~s(<%= MDF.number_input f, #{inspect(key)}, input: [step: "any"], label: [text: gettext\("#{key}"\)] %>)
      {key, :boolean} ->
        ~s(<%= MDF.checkbox f, #{inspect(key)}, label: [text: gettext\("#{key}"\)] %>)
      {key, :text} ->
        ~s(<%= MDF.textarea f, #{inspect(key)}, label: [text: gettext\("#{key}"\)] %>)
      {key, :date} ->
        ~s(<%= MDF.date_select f, #{inspect(key)}, label: [text: gettext\("#{key}"\)] %>)
      {key, :time} ->
        ~s(<%= MDF.time_select f, #{inspect(key)}, label: [text: gettext\("#{key}"\)] %>)
      {key, :utc_datetime} ->
        ~s(<%= MDF.datetime_select f, #{inspect(key)}, label: [text: gettext\("#{key}"\)] %>)
      {key, :naive_datetime} ->
        ~s(<%= MDF.datetime_select f, #{inspect(key)}, label: [text: gettext\("#{key}"\)] %>)
      {key, {:array, :integer}} ->
        ~s(<%= MDF.multiple_select f, #{inspect(key)}, ["1": 1, "2": 2], label: [text: gettext\("#{key}"\)] %>)
      {key, {:array, _}} ->
        ~s(<%= MDF.multiple_select f, #{inspect(key)}, ["Option 1": "option1", "Option 2": "option2"], label: [text: gettext\("#{key}"\)] %>)
      {key, _}  ->
        ~s(<%= MDF.text_input f, #{inspect(key)}, label: [text: gettext\("#{key}"\)] %>)
    end)
  end
end
